/*jslint node:true */
var crypto = require('crypto'), exports = module.exports;

var sessionKey = null;

exports.init = function(sk) {
	sessionKey = sk;
};
exports.generate = function(name,password,expiry) {
	var token, sha1 = crypto.createHash('sha1');
	token = [name,password,sessionKey,expiry].join(":");
	sha1.update(token);
	token = exports.cipher([sha1.digest('hex'),name,expiry].join(":"));
	return(token);
};

exports.cipher = function(token) {
	var cipher = crypto.createCipher('aes192', sessionKey);
	token = cipher.update(token, 'utf8','hex');
	token += cipher.final('hex');
	return token;
};

exports.decipher = function(token){
	var decipher = crypto.createDecipher('aes192', sessionKey);
	token = decipher.update(token, 'hex','utf8');
	token += decipher.final('utf8');
	return token;
};

exports.validate = function(token,name,password) {
	var p, sec, valid = false, expiry, sha1 = crypto.createHash('sha1');
	token = token || "";
	p = token.split(":");
	expiry = p[2];
	if (p[1] === name && !isNaN(expiry) && parseInt(expiry,10) > new Date().getTime()) {
		sec = [name,password,sessionKey,expiry].join(":");
		sha1.update(sec);
		sec = sha1.digest('hex');
		valid = sec === p[0];
	}
	return(valid);
};
