...
var fs = require('fs'), vm = require('vm'), _ = require('lodash'), async = require('async'),
errors = require('./errors'), csauth = "X-CS-Auth",
/* 
...
module.exports = {
	...
	loadFile: function (cfile) {
		...
		return function(req,res,next) {
			...
			var authenticated = true, authorized = true, initialized = true, user = req[csauth], keys, match, oldParams = req.params;
			...
			async.each(routes[req.method.toLowerCase()],function (entry,callback) {
				var useRule = false, checkCondition = function (condition,req,user,item) {
					var authorized;
					try {
						authorized = vm.runInNewContext(condition,{req:req,request:req,user:user,_:_,item:item});
					} catch (e) {
						authorized = false;
					}
					return(authorized);
					...
					if (useRule) {
						...
						if (authenticated) {
							...
							if (entry.loader) {
								try {
									...
									loader[entry.loader](req,res,function (err) {
										if(err) {
											...
											authorized = checkCondition(entry.condition,req,user,(req.cansecurity||{}).item);
											callback();
										}
									});
								} catch (err) {
								...
								authorized = checkCondition(entry.condition,req,user);
								callback();
							}
						} else {
							callback();
				...
				if (!initialized) {
					res.send(500,errors.uninitialized());
